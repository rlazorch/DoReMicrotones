...
public class PlayerMovement : MonoBehaviour {
	...
	void Start() {
		osc.SetAddressHandler("/WalkRight", WalkRight);
	}
	...
	void WalkRight(OscMessage message) {
		
		int note = message.GetInt(0);
		if (note == noteExpected) {
			noteExpected *= -1;
			//horizontalMove = runSpeed * 50;
			dash = true;
		}
		Debug.Log("Heard" + note.ToString());
	}
